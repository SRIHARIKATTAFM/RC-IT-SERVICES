...
async function handleApi(request) {
  ...
  if (request.method === 'GET' && action === 'health') {
    return json(200, { ok: true, service: 'rc-it-services', runtime: 'cloudflare-workers', now: new Date().toISOString() });
  }

  if (action === 'login') {
    return json(501, { ok: false, code: 'AUTH_NOT_CONFIGURED', message: 'Portal authentication is intentionally not enabled in this build.' });
  }
  ...
  if (action === 'resume' || action === 'career-application') {
    return json(501, { ok: false, code: 'RECRUITMENT_STORAGE_NOT_CONFIGURED', message: 'Recruitment document submission will be enabled after approved private storage is connected.' });
  }
