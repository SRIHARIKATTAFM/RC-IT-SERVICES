...
async function handleApi(request) {
  ...
  if (action === 'login') {
    return json(501, {
      ...
      message: 'Portal authentication is intentionally not enabled in this build.'
    });
  }
  ...
  if (action === 'contact') {
    ...
    return json(202, {
      ...
      message: 'Enquiry validated. Production email or CRM delivery will be connected before public launch.'
    });
  }

  if (action === 'demo') {
    if (!clean(payload.name, 120) || !clean(payload.company, 140) || !clean(payload.businessEmail, 254) || !clean(payload.product, 120)) {
      return json(422, { ok: false, message: 'Name, company, business email and product are required.' });
    }
  ...
  if (action === 'consultation') {
    if (!clean(payload.name, 120) || !clean(payload.company, 140) || !clean(payload.businessEmail, 254) || !clean(payload.topic, 140)) {
      return json(422, { ok: false, message: 'Name, company, business email and consultation topic are required.' });
    }
    if (!isEmail(payload.businessEmail)) return json(422, { ok: false, message: 'Enter a valid business email.' });
    if (payload.phone && !isPhone(payload.phone)) return json(422, { ok: false, message: 'Enter a valid phone number.' });
  ...
  if (action === 'chat') {
    if (!clean(payload.name, 120) || !clean(payload.businessEmail, 254) || !clean(payload.message, 3000)) {
      return json(422, { ok: false, message: 'Name, business email and message are required.' });
    }
    if (!isEmail(payload.businessEmail)) return json(422, { ok: false, message: 'Enter a valid business email.' });
    return json(202, { ok: true, id: crypto.randomUUID(), message: 'Message validated successfully.' });
  }

  if (action === 'resume' || action === 'career-application') {
    return json(501, {
      ok: false,
      code: 'RECRUITMENT_STORAGE_NOT_CONFIGURED',
      message: 'Recruitment document submission will be enabled after approved private storage is connected.'
    });
  }
